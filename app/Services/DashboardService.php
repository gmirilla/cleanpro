...
class DashboardService
{
     ...
     * Call DashboardService::clearCache() after any booking/payment change.
     */
    ...
    public function getStaffStats(?int $staffId): array
    {
        ...
        // Staff stats cached per staff member for 2 minutes
        return Cache::remember("staff_stats_{$staffId}", now()->addMinutes(2), function () use ($staffId) {
            return [
    ...
    public function getCustomerStats(?int $customerId): array
    {
        ...
        return Cache::remember("customer_stats_{$customerId}", now()->addMinutes(2), function () use ($customerId) {
            return [
    ...
    /**
     * Call this whenever bookings or payments change so dashboards show fresh data.
     */
    public static function clearCache(?int $staffId = null, ?int $customerId = null): void
    {
        Cache::forget('admin_dashboard_stats');

        if ($staffId) {
            Cache::forget("staff_stats_{$staffId}");
        }
