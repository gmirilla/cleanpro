<div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-6 flex flex-wrap gap-3">
        <input wire:model.live.debounce.300ms="search"
               type="text" placeholder="Search by reference, customer…"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Reference</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Customer</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Services</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Service Date</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Amount</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Staff</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Status</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Invoice</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($bookings as $booking)
                        ...
                        <tr class="hover:bg-gray-50 transition {{ $viewingId === $booking->id ? 'bg-indigo-50' : '' }}">
                            ...
                            <td class="px-4 py-3">
                                <span class="px-2 py-1 rounded-full text-xs font-semibold bg-{{ $c }}-100 text-{{ $c }}-700 whitespace-nowrap">
                                    {{ ucfirst(str_replace('_', ' ', $booking->status)) }}
                                </span>
                            </td>
                            <td class="px-4 py-3">
                                @if($booking->invoice)
                                    <span class="text-xs {{ $booking->invoice->isPaid() ? 'text-green-600 font-semibold' : 'text-orange-500' }}">
    ...
    <div wire:click="closeDrawer"
         class="fixed inset-0 z-40 bg-black/30 backdrop-blur-sm transition-opacity"></div>
        ...
        <div class="flex items-start justify-between px-6 py-5 border-b border-gray-200 bg-gray-50">
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-{{ $bc }}-100 text-{{ $bc }}-700">
                        {{ ucfirst(str_replace('_', ' ', $b->status)) }}
                    </span>
                    @if($b->invoice)
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $b->invoice->isPaid() ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }}">
            ...
            <button wire:click="closeDrawer" class="text-gray-400 hover:text-gray-700 text-2xl leading-none mt-1">&times;</button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-6">
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @elseif($b->invoice)
                        <span class="inline-flex items-center gap-1.5 bg-gray-100 text-gray-500 text-sm font-medium px-4 py-2 rounded-lg">
                    ...
                    @foreach($b->items as $item)
                        <div class="flex justify-between items-center px-4 py-3">
                            <div>
                                ...
                                <p class="text-xs text-gray-400">{{ ucfirst($item->service->category ?? '') }} · Qty: {{ $item->quantity }} × ₦{{ number_format($item->price, 2) }}</p>
                            </div>
                            <span class="font-semibold text-gray-700 text-sm">₦{{ number_format($item->subtotal, 2) }}</span>
                        </div>
                    @endforeach
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    ...
                    <p class="font-semibold text-gray-800 text-sm">{{ $b->pickup_date->format('D, d M Y') }}</p>
            ...
            <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div class="px-4 py-3 space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center pt-1">
                        <span class="text-gray-400 text-xs">Due: {{ \Carbon\Carbon::parse($b->invoice->due_date)->format('d M Y') }}</span>
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold
                            {{ $b->invoice->status === 'paid' ? 'bg-green-100 text-green-700' :
                               ($b->invoice->status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700') }}">
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div class="text-center mb-4">
                ...
                <p class="text-sm text-gray-500 mt-1">Optionally provide a reason. The customer will be notified.</p>
            </div>
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button wire:click="$set('assigningId', null)" class="text-gray-400 hover:text-gray-700 text-xl">&times;</button>
            </div>
            <select wire:model="selectedStaff"
                ...
                @foreach($availableStaff as $s)
                    <option value="{{ $s->id }}">{{ $s->user->name }} — {{ ucfirst($s->role ?? 'Staff') }}</option>
                @endforeach
            </select>
            @error('selectedStaff')
